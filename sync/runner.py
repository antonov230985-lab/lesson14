...
def run_once(config: AppConfig, yandex: YandexDriveClient, google: GoogleDriveClient, state_store: StateStore) -> None:
    ...
    yandex_meta = None
    if config.yandex_token and config.remote_input_path_yandex:
        yandex_meta = yandex.get_file_meta(config.remote_input_path_yandex)

    google_input = google.find_file_by_name(config.google_input_name, config.google_input_folder_id)
    if not google_input:
        logger.info(
            "Google input file '%s' not found in folder '%s'. Skip.",
            config.google_input_name,
            config.google_input_folder_id or "root",
        )
        return

    google_input_file_id = str(google_input["id"])
    google_meta = google.get_file_meta(google_input_file_id)
    decision = detect_change(yandex_meta.version if yandex_meta else None, google_meta.version, state)
